...
@php
    $member_auth = Session::get('member_data');
@endphp
...
<script>
    ...
    @if (isset($member_auth) && !empty($member_auth))

        function getUnreadNotifications(){
            
            var formData = new FormData();            
            formData.append('user_id', '{{ $member_auth->id }}');
            formData.append('user_type', 'member');

            window.getResponseInJsonFromURL("{{ route('member.notifications.unread_list') }}", formData, (response) => {
        ...
        $('body').on('click','#notify-all-read-btn',function(){
            ...
            nformData.append('user_id', '{{ $member_auth->id }}');
            nformData.append('user_type', 'member');

            getResponseInJsonFromURL("{{ route('member.notifications.read_all') }}", nformData, (response) => {
